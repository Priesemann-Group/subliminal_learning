...
set -euo pipefail
SEEDS=$(seq 0 19)
OUTDIR="./outputs/student_aux_head_perturb"
mkdir -p "$OUTDIR"
...
for seed in $SEEDS; do
  for PSTD in "${PERTURB_STDS[@]}"; do
    LABEL=$(python - <<PY
p = float("${PSTD}")
print(f"student_aux_head_perturb_{p:.8f}".replace(".", "p"))
PY
)
    ...
    python ../../src/run_subliminal.py \
      ...
      --data-dir ../../MNIST \
      --seed "$seed" \
      --sweep-name "mlp_student_aux_head_perturb_sweep" \
      --run-label "$LABEL" \
      --dataset mnist \
      --teacher-type mlp \
      --student-type mlp \
      ...
      --noise-steps 60 \
      --noise-dist uniform \
      --perturb "student:aux_head,std=${PSTD},timing=before_student_training,include_weight=true,include_bias=true"
  done
done